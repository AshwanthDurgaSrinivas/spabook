...
import { serviceService, type ServiceCategory, type Service } from '@/services/serviceService';
...
export function EditServicePage() {
    ...
    const fetchService = useCallback(async () => {
        ...
        try {
            ...
            setCapacity((service.capacity ?? 1).toString());
            if (service.imageUrls && service.imageUrls.length > 0) {
                setImageUrl(service.imageUrls[0]);
                setImagePreview(service.imageUrls[0].startsWith('http') ? service.imageUrls[0] : `http://localhost:5000${service.imageUrls[0]}`);
            }
        } catch (error) {
    ...
    useEffect(() => {
        Promise.all([fetchCategories(), fetchService()]);
    }, [fetchCategories, fetchService]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setIsLoading(true);
        try {
            await serviceService.updateService(parseInt(id), {
                name,
    ...
    return (
        <div className="space-y-6 max-w-4xl mx-auto pb-12">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-3xl font-display font-bold text-gray-800">Edit Service</h1>
                    <p className="text-gray-500">Update treatment details and pricing</p>
                </div>
            </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="flex gap-3">
                        ...
                        <Button type="submit" className="flex-1 gradient-coral text-white" disabled={isLoading || isUploading}>
                            {isLoading ? 'Updating...' : isUploading ? 'Uploading Image...' : 'Save Changes'}
                        </Button>
                    </div>
                </div>
            </form>
